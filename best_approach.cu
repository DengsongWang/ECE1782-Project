...
__global__ void motionKernel(unsigned char *d_curr_frame, unsigned char *d_ref_frame,
...
void block_match_full_frame_stream_save_reconst(Frame *allYFrames, unsigned char *allRconstFrames)
{
    ...
    for (int i = 0; i < STREAM_NUM; i++)
    {
        ...
        motionKernel<<<blocks, threadsPerBlock, 0, stream[i]>>>(d_curr_frame[i], d_ref_frame[i], d_temp_frame[i], d_motion_vectors[i]);
